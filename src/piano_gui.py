...
class PianoGUI:
    ...
    def update_keys_set(self): # 키설정 업데이트
        self.left_dict = {'Z': f'C{self.left_oct}',
                          'S': f'Cs{self.left_oct}',
                          'X': f'D{self.left_oct}',
                          'D': f'Ds{self.left_oct}',
                          'C': f'E{self.left_oct}',
                          'V': f'F{self.left_oct}',
                          'G': f'Fs{self.left_oct}',
                          'B': f'G{self.left_oct}',
                          'H': f'Gs{self.left_oct}',
                          'N': f'A{self.left_oct}',
                          'J': f'As{self.left_oct}',
                          'M': f'B{self.left_oct}'}
        self.right_dict = {'W': f'F{self.right_oct - 1}',
                           '3': f'Fs{self.right_oct - 1}',
                           'E': f'G{self.right_oct - 1}',
                           '4': f'Gs{self.right_oct - 1}',
                           'R': f'A{self.right_oct - 1}',
                           '5': f'As{self.right_oct - 1}',
                           'T': f'B{self.right_oct - 1}',
                           'Y': f'C{self.right_oct}',
                           '7': f'Cs{self.right_oct}',
                           'U': f'D{self.right_oct}',
                           '8': f'Ds{self.right_oct}',
                           'I': f'E{self.right_oct}',
                           'O': f'F{self.right_oct}',
                           '0': f'Fs{self.right_oct}',
